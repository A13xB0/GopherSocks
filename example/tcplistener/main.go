...
func main() {
	...
	host := "0.0.0.0"
	port := uint16(8001)
	tListener, err := gophersocks.NewTCPListener(host, port)
	if err != nil {
		panic(err)
	}
	...
	go stop(tListener, sigs)
	//Use stream handler announce to handle new sessions (not neccessary but adds simplicity)
	tListener.SetAnnounceNewSession(announceStreamHandler, nil)

	//Start Listening
	fmt.Printf("Starting listener on %s:%d\n", host, port)
	if err := tListener.StartReceiveStream(); err != nil {
		panic(err)
	}
}
